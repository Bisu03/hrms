...
const Qrlist = () => {
    ...
    const HandleSubmit = async () => {
        setLoading(true)
        try {
            const { data } = await apiRequest.post("/api/attendence/generateqr", {
                date: GetDate()
            })
            router.push(`/attendence/qr/${data._id}`)
            setLoading(false)
        } catch (error) {
            setLoading(false)
                                    ...
                                    <tr key={key}>
                                        ...
                                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                            <div className="flex items-center space-x-3.5">
                                                <Link href={`/attendence/qr/${data._id}`} className="hover:text-primary">
                                                    <svg
