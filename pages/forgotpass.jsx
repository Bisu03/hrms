...
const Forgotpass = () => {
    ...
    const [EmloyeeEmail, setEmloyeeEmail] = useState("");

    const HandleSubmit = async () => {
        setLoading(true);
        try {
            const { data } = await apiRequest.post("/api/user/resetpass/reset", {
                email: EmloyeeEmail,
            });
            if (data.notfound) {
                toast.error("Email Not Found");
            } else {
                router.push("/resetpass?id=" + data);
                setLoading(false);
                SuccessHandling("otp send to your email");
            }
        } catch (error) {
            setLoading(false);
    ...
    return (
        <div className="h-screen flex items-center justify-center w-full bg-boxdark">
            <div className="rounded-sm border border-stroke text-white shadow-default dark:border-strokedark bg-boxdark  ">
                                ...
                                <div className="mb-4">
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            name="email"
                                            value={EmloyeeEmail}
                                            onChange={(e) => setEmloyeeEmail(e.target.value)}
                                            required
