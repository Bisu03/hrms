...
const Permission = () => {
    ...
    const HandleFetchData = async () => {
        try {
            const { data } = await apiRequest.get(`/api/user/details/${slug}`)
    ...
    const HandleSavePermission = async () => {
        if (window.confirm('Are you sure you want to save this?')) {
            setLoading(true)
            try {
                const { data } = await apiRequest.put(`/api/user/details/${slug}`, { permission: Permission?.permission })
                setLoading(false)
        ...
        <DefaultLayout>
            ...
            <div className="col-span-12 xl:col-span-7">
                <div className=" rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6  shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
                    <h1>Grant Access To {Permission?.employee_name}</h1>
                    <div className="flex flex-wrap w-full justify-start my-5 ">
                        ...
                        <span
                            onClick={() => HandleAddPermission("Role Management")}
                            className={`my-2 cursor-pointer ${Permission?.permission?.includes("Role Management") ? "bg-red" : "bg-primary"} text-white text-xs font-medium me-2 px-2.5 py-2 rounded   border border-blue- flex items-center`}>
                            {Permission?.permission?.includes("Role Management") && <CgClose />}  Role Management
                        </span>
                    </div>
